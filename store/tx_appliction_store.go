...
type txApplicationStore struct {
	tx *sql.Tx
}

// Create inserts a new application in the context of a transaction
func (t *txApplicationStore) Create(ctx context.Context, app *domain.Application) error {
...
func (t *txApplicationStore) Exists(ctx context.Context, jobID, userID int64) (bool, error) {
	var exists int
	query := `
	SELECT 1
	FROM applications
